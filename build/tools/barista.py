...
from xml.etree.ElementTree import Element, ElementTree, ParseError, indent, parse
...
def grind_beans(device: str) -> list[Element]:
    ...
    beans_manifest = parse(beans_path).getroot()
    projects = {}

    def process_manifest(manifest: Element) -> None:
        projects.update(
            {
    ...
    for include_elem in beans_manifest.findall("include"):
        include_path = beans_path.parent / include_elem.get("name", "")
        try:
            include_manifest = parse(include_path).getroot()
            process_manifest(include_manifest)
        except FileNotFoundError:
            print(f"Warning: Include beans not found at {include_path}. Skipping.")
        except ParseError:
            print(
                f"Warning: Unable to grind include beans at {include_path}. Skipping."
            )

    # Process main file, overriding include file entries
    process_manifest(beans_manifest)
...
def main():
    ...
    for project in projects:
        ...
        if existing is None:
            ...
            updated_attrs = [
                attr
                for attr in ["name", "remote", "revision"]
                if existing.get(attr) != project.get(attr)
            ]
